...
public class ConsoleMenu {
    ...
    PetControler petControler;

    public ConsoleMenu(PetControler controller) {
    ...
    public void start() {
        ...
        try (Scanner in = new Scanner(System.in, "ibm866"); Counter count = new Counter()) {
            ...
            while (flag) {
                ...
                switch (key) {
                    case "1":
                        petControler.getAllPet();
                        break;
                    case "2":
                        PetType type = menuChoice(in);
                        if (type != null) {
                            try {
                                petControler.createPet(type);
                                count.add();
    ...
    private int menuChoicePet(Scanner in) {
        ...
        while (true) {
            ...
            if (id == 0)
                return id;
            if (petControler.getById(id) == null) {
    ...
    private void menuTrainPet(int petId, Scanner in) {
        ...
        while (true) {
            ...
            if (command.length() == 1 && command.equals("0"))
                return;
            if (petControler.trainPet(petId, command))
